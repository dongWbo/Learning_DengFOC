...
void Encoder::c(int32_t count, bool update_offset) {
    // Disable interrupts to make a critical section to avoid race condition
    uint32_t prim = cpu_enter_critical();
...
bool Encoder::run_offset_calibration() {
    ...
    if (axis_->motor_.config_.motor_type == Motor::MOTOR_TYPE_HIGH_CURRENT)//大电流电机的校准电压 = 校准电流 * 相电阻
    ...
    axis_->run_control_loop([&](){
        ...
        return ++i < start_lock_duration * current_meas_hz;////////////////////////////////////////////////////////////////归位1秒钟
    });
